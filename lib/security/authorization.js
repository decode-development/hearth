  ...
  return {
    ...
    authorize: (interaction, ctx, resourceType, callback) => {
      ...
      if (!authorizer || !authorizer.allowedResourceTypes) {
        logger.debug('Access Denied: No authorizer or allowedResourceTypes')
      ...
      if (resourceType === 'user' && !isUserManagementRequestValid(interaction, ctx, authorizer)) {
        logger.debug('Access Denied: Invalid user management request')
        return sendForbidden()
      }
      ...
      sendForbidden()
      logger.debug('Allowed Resource Types:', authorizer.allowedResourceTypes)
    }
  }
}
